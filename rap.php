...
require("setup.php");
$rok = $_GET['rok'];
$c->query("create table if not exists {$rok}_podstawa_wnioski(id int, wniosek text, primary key(id))");
     $test = $c->query("select * from {$rok}_podstawa_wnioski");
    if(@$test->num_rows==0)
    {
        $c->query("insert into {$rok}_podstawa_wnioski values(1, 'Wniosek: ')");
    }

$c->query("drop table {$rok}_wyniki_klas");
$name = $c->query("show tables like '{$rok}%p'");
$i=0;
?>
<main>
    <table>
...
//            $c->query("create table if not exists wyniki_klas(wynik float)");
            ...
            $sql = "create table {$rok}_wyniki_klas(id int not null auto_increment primary key, wynik float) ";
//            $c->query($sql);
...
//            echo $sql;
        ...
        ?>
            <?php
            $result = $c->query("select count(wynik) as result from {$rok}_wyniki_klas where wynik<0.3");
            $overall = $c->query("select count(wynik) as overall from {$rok}_wyniki_klas");
...
//    echo "Do egzaminu maturalnego  z  matematyki  na  poziomie rozszerzonym w ZSE przystąpiło $roz uczniów. Wynik  z poziomu  rozszerzonego  nie decydował o zdawalności egzaminu maturalnego.";								
    ...
    <table>
        ...
        for($j = 0; $j<$i; $j++)
        {
            ...
            $xdd = $c->query("select round(avg(wynik), 4) as avg from {$rok}_wyniki_klas");
...
//            $xd = $result->fetch_array();
        ...
        <tr>
            ...
            #####################
            $wklas = $c->query("select median(wynik) as md from {$rok}_wyniki_klas");
        ...
        <tr>
            ...
            #####################
            $wklas = $c->query("select stats_mode(wynik) from {$rok}_wyniki_klas");
            while($row = $wklas->fetch_array())
            {
                echo '<td>';
        ...
        <tr>
            ...
            $wklas = $c->query("select round(max(wynik),2) - round(min(wynik),2) as rozstep from {$rok}_wyniki_klas");
            while($row = $wklas->fetch_array())
            {
                echo '<td>';
                 $mediana = $row['rozstep'];
...
//                $max = substr(($l[$j]['max'] * 0.5 * 100), 0, 2);
            ...
            $zse = $c->query("select round(max(wynik), 2) as max from {$rok}_wyniki_klas");
            ...
            for($j = 0; $j<$i; $j++)
        {
            ...
            $zse = $c->query("select round(min(wynik), 2) as max from {$rok}_wyniki_klas");
            while($row = $zse->fetch_array())
            {
        ...
        <tr>
            ...
            #####################
            $zse = $c->query("select stddev(wynik) from {$rok}_wyniki_klas");
            while($row = $zse->fetch_array())
            {
                echo '<td>';
...
?>
        ...
        <tr>
            ...
            #####################
            $zse = $c->query("select avg(wynik)-stddev(wynik), avg(wynik)+stddev(wynik) from {$rok}_wyniki_klas");
            while($row = $zse->fetch_array())
            {
                echo '<td>(';
...
-->
    ...
    <?php
    $result = $c->query("SELECT round(wynik*0.5*100) as pkt, COUNT(*) AS ilosc FROM {$rok}_wyniki_klas GROUP BY pkt ORDER BY pkt desc");
...
//    echo $result['pkt'] $result['ilosc'];
    ...
    $result = $c->query("select count(wynik) AS ilosc FROM {$rok}_wyniki_klas where wynik > 0.29 ");
    $res = $c->query("select count(wynik) as ogol FROM {$rok}_wyniki_klas ");
 $result=$result->fetch_array();
 $res=$res->fetch_array();
    echo "<p>Wynik powyżej 30% osiągnęło {$result['ilosc']} osób, co stanowi " . round($result['ilosc'] / $res['ogol'] *100, 2)."% wszystkich osób</p>";


    $result = $c->query("select count(wynik) AS ilosc FROM {$rok}_wyniki_klas where wynik < 0.30 ");
    $res = $c->query("select count(wynik) as ogol FROM {$rok}_wyniki_klas ");
 $result=$result->fetch_array();
 $res=$res->fetch_array();
    echo "<p>Wynik poniżej 30% osiągnęło {$result['ilosc']} osób, co stanowi " . round($result['ilosc'] / $res['ogol'] *100, 2)."% wszystkich osób</p>";

    $xxx= $c->query("SELECT stats_mode(wynik) from {$rok}_wyniki_klas");    
    $xxx= $xxx->fetch_array();
    ...
    $ile= $c->query("SELECT count(wynik) from {$rok}_wyniki_klas where wynik like $iledome");    
...
<form action="wniosekrap.php" method="post">
